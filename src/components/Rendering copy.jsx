...
import ObjToPrimitive from "./BasicObjLoader";
...
export default function Rendering() {
  ...
  console.log(ObjToPrimitive({url: objs[objName].url, mat}))
        ...
        <Card.Body>
          <Card.Title>Basic Triangular Mesh</Card.Title>
            ...
            <Canvas camera={{ position: [0, 5, 0], fov: 40}}>
              ...
              <mesh position={[0, 0, 0]} scale={[6,6,6]} >
                {ObjToPrimitive({url:'bunny.obj', mat})}
              </mesh>
            </Canvas>
            </div>
          <Card.Footer>
            ...
            <Dropdown.Menu >
              {
                Object.keys(objs).map((objName) => {
                  return <Dropdown.Item key={objName} onSelect={()=>handleSelect(objName)}>{objName}</Dropdown.Item>
                })
              }
